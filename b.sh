...
if ! command -v curl &> /dev/null; then
    echo "[!] Chưa có curl -> cài..."
    pkg install curl -y
fi

if ! command -v python &> /dev/null; then
    echo "[!] Chưa có python -> cài..."
...
for url in "${LINKS[@]}"; do
    ...
    if [ -f "$filename" ]; then
        ...
        if command -v su &> /dev/null; then
            su -c "pm install -r '$DOWNLOAD_PATH/$filename'" 2>/dev/null
        else
            echo "⚠️ Không có root -> bỏ qua cài"
        fi
    else
        echo "❌ Tải lỗi: $filename"
    fi
done
...
# ===== LOAD TOOL =====
echo "[*] Lấy tool GitHub..."
curl -s https://raw.githubusercontent.com/zam2109/Toolrejoin/refs/heads/main/Setubtoolrejoin | bash
